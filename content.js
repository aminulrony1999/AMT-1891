...
function click() {
  const jobsNumber = document.querySelectorAll(".jobslisthighlight")?.length;
  const jobsList = document.querySelectorAll(".jobslisthighlight");
  if(jobsNumber > 0)
  {
    for(let i = 0; i < jobsNumber; i++)
    {
      if(jobsList[i].querySelector("a")?.innerText == 'TTV-Data Entry from images (E1096-1891)')
      {
        jobsList[i]?.querySelector("a")?.click();
        break;
      }
    }
  }
...
function acceptAndStart() {
  ...
  console.log(jobId);
  //setting Job Id to local storage
